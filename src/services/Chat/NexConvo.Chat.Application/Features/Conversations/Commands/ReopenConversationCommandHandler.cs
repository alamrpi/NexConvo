...
public sealed class ReopenConversationCommandHandler(
    ...
    : IRequestHandler<ReopenConversationCommand, Result>
{
    public async Task<Result> Handle(ReopenConversationCommand cmd, CancellationToken ct)
    {
        ...
        // Conversation.Reopen throws InvalidConversationStateTransitionException if not
        // Resolved/Closed — let it propagate; the controller translates it to 409 Conflict.
        conversation.Reopen();

        await db.SaveChangesAsync(ct);

        logger.LogInformation(
            "Agent {ActorUserId} reopened conversation {ConversationId}",
            cmd.ActorUserId, cmd.ConversationId);

        // Fire after successful persistence; IChatEventPublisher is best-effort and never throws.
        await eventPublisher.PublishReopenedAsync(tenantId, conversation.Id, conversation.UpdatedAt, ct);
