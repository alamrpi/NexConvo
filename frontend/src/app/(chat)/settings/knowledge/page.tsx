...
import { Plus, Eye, RefreshCw, Trash2, FileText, Link, MessageSquare, File } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
      ...
      <DialogContent className="max-w-lg w-full">
        ...
        <Tabs defaultValue="upload">
          ...
          <TabsContent value="upload" className="mt-4 space-y-3">
            ...
            >
              <p className="text-sm font-medium text-foreground">{t('knowledge.upload.hint')}</p>
              <p className="mt-1 text-xs text-muted-foreground">{t('knowledge.upload.types')}</p>
              <input
                id="kb-file-input"
                type="file"
            ...
            {uploadFile && !uploadError && (
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {uploadDone ? 'Complete' : `${t('knowledge.upload.uploading')} ${uploadProgress}%`}
                </p>
              </div>
            )}
          ...
          <TabsContent value="url" className="mt-4 space-y-3">
            <Label htmlFor="kb-url">{t('knowledge.url.label')}</Label>
            <div className="flex gap-2">
              <Input
                id="kb-url"
                type="url"
                placeholder={t('knowledge.url.placeholder')}
                value={url}
                onChange={e => { setUrl(e.target.value); setUrlReady(false); }}
              />
              <Button variant="outline" onClick={fetchUrl} disabled={urlLoading || !url.trim()}>
                {urlLoading ? 'Fetching…' : t('knowledge.url.fetch')}
              </Button>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer text-sm">
                <input
                  ...
                  className="accent-primary"
                />
                {t('knowledge.text.qaBuilder')}
              </label>
            </div>

            {textMode === 'free' ? (
              <div className="space-y-2">
                <Label htmlFor="kb-free-title">{t('knowledge.text.titleLabel')}</Label>
                <Input
                  id="kb-free-title"
                  value={freeTitle}
                ...
                />
                <Label htmlFor="kb-free-text">{t('knowledge.text.label')}</Label>
                <Textarea
                  id="kb-free-text"
                  rows={6}
                  value={freeText}
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                {qaPairs.map((pair, idx) => (
                  <div key={pair.id} className="rounded-md border border-border p-3 space-y-2">
                    ...
                    <Input
                      placeholder={t('knowledge.text.question')}
                      value={pair.question}
                      onChange={e => updateQaPair(pair.id, 'question', e.target.value)}
                    />
                    <Textarea
                      rows={2}
                      placeholder={t('knowledge.text.answer')}
                      value={pair.answer}
            ...
            {docs.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="py-12 text-center text-muted-foreground text-sm">
                  No knowledge sources yet. Add your first document.
                </TableCell>
              </TableRow>
            )}
            {docs.map(doc => (
                ...
                <TableCell>
                  <div className="flex items-center justify-end gap-1">
                    <Button
                      ...
                      onClick={() => router.push(`/settings/knowledge/${doc.id}`)}
                    >
                    ...
                    <div className="relative">
                      ...
                      {reEmbedding[doc.id] && (
                        <span className="absolute right-8 top-0 whitespace-nowrap text-xs text-muted-foreground">
                          {t('knowledge.reEmbedding')}
                        </span>
                      )}
                    </div>
                    <Button
