...
| 22 | **Integration connection health** | Any integration that stores workspace credentials (S3, AI provider, channels, SMS/email gateways, payment/webhooks — anything with a key that can expire/rotate/revoke) MUST: **(a) Test-then-save** — a live "Test Connection" endpoint the user runs first; Save is a separate action enabled only after a green test, and the server re-tests changed credentials and rejects with **422** on failure. **(b) Persist health** — `LastTestedAt / LastTestStatus (ConnectionStatus) / LastTestError / LastTestLatencyMs` on the config entity via the shared `IConnectionTester<TInput>` + `ConnectionHealth` abstraction (BuildingBlocks). **(c) Guard dependent features** — before using a config, `EnsureHealthy(...)`; if not `Healthy`, refuse with **409 ConnectionUnhealthy** and disable+warn in the UI. **(d) Scheduled re-test + alert** — a trigger-only central scheduler (Automation, no secrets) fans out `CheckIntegrationHealthCommand`; the **owning** service tests its own configs (decrypted secrets NEVER cross the network) and on a `Healthy→Failed` transition publishes `IntegrationHealthFailedEvent`; the Notification service emails owner+admins (fetching contacts from Identity — email-provider secrets live only in Notification). Never ship a credential-storing integration without test-then-save, health persistence, and a feature guard. See `docs/superpowers/specs/2026-07-07-connection-health-design.md`. |
...
- "It saves the key, that's enough — I'll skip the test/health" → **test-then-save + persist health + guard the feature (Standard 22)**
- "Test can just save it directly" → **Test and Save are separate steps; Save is enabled only after a green test (Standard 22)**
- "The feature can just use whatever config is saved" → **guard with `EnsureHealthy`; refuse 409 on an unhealthy config (Standard 22)**
- "The health job can decrypt and test other services' keys centrally" → **secrets never leave the owning service; scheduler only triggers (Standard 22)**
